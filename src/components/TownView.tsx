...
export const TownView: React.FC<TownViewProps> = ({
  ...
  const hasGeneralStore = 
    locType !== "marked_grave" && locType !== "ephemeral_stash" && locType !== "hostile_camp"; 
  ...
  const hasBanditBoss = 
    locType !== "marked_grave" && (locType === "outlaw_haven" || locType === "hostile_camp");
          ...
          >
            ...
            <span>
              {location.type === "ephemeral_stash"
                ? "Stash Cache"
                : location.type === "marked_grave"
                  ? "Grave Site"
                  : "Town Square"}
            </span>
          </button>

          {location.type !== "ephemeral_stash" && location.type !== "marked_grave" && (
            <>
              {hasSaloon && (
                <button
                  id="tab-saloon"
            ...
            <div className="space-y-4 flex-1">
              <div className="bg-[#e8dec7] border border-[#bfae96] p-4 rounded-sm flex flex-col justify-center items-center text-center space-y-4 min-h-[300px]">
                ...
                <p className="text-[#4a3928] max-w-sm text-sm italic font-serif">
                  {location.type === "mine"
                    ...
                    : location.type === "ephemeral_stash"
                      ...
                      : location.type === "marked_grave"
                        ? (location.description || `A marked grave sits quietly in the wind. The tombstone reads: Here lies the body of a fallen drifter.`)
                ...
                <div className="w-full max-w-md bg-[#dcd1b9] p-3 rounded-sm border border-[#bfae96]/60 flex flex-col gap-2 mx-auto">
                  {location.type === "marked_grave" && onLootGrave && (
                    <div className="space-y-3">
                      {location.graveLooted ? (
                        <p className="text-xs text-[#664d36] italic py-2">
                          ✝️ You have paid your respects. There is nothing left but dry desert dust and quiet memories.
                        </p>
                      ) : (
                        <div className="space-y-2">
                          <p className="text-[11px] text-[#4a3928] font-sans">
                            Buried with <strong className="text-[#8c6b0c]">{location.graveInventory?.length || 0}</strong> belongings from a previous life. You can scavenge <strong>1 random item</strong> as your inheritance.
                          </p>
                          <button
                            onClick={() => onLootGrave(location.id)}
                            className="w-full py-3 bg-[#5c4033] hover:bg-[#3e2a1e] border border-[#bfae96] text-[#f4ead5] font-bold text-xs font-serif uppercase tracking-wider rounded transition-colors"
                          >
                            🪦 Salute & Scavenge 1 Inherited Item
                          </button>
                        </div>
                      )}
                    </div>
              ...
              {location.type !== "marked_grave" && location.type !== "ephemeral_stash" && (
                  ...
                  {player.bounty && player.bounty > 0 ? (() => {
                    ...
                    return (
                      <div className="bg-[#e6d8b8] border-8 border-[#cfbfa0] border-double p-5 rounded-sm max-w-sm mx-auto text-stone-900 shadow-xl font-serif flex flex-col items-center text-center relative overflow-hidden my-2">
                        ...
                        {/* Dynamic Pursuit tracker details inside pursuit poster */}
