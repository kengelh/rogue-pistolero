...
export const getBiomeAt = (x: number, y: number) => {
  ...
  if (plainsVal > 0.3) {
    const isLush = plainsVal > 0.7;
    const isDry = plainsVal < 0.5;
    return {
      ...
      hydrationModifier: 1.0,
      color: isLush
        ? "text-green-300 bg-green-800 border-green-700"
        : isDry
          ? "text-lime-300 bg-lime-900 border-lime-800"
          : "text-emerald-400 bg-emerald-900 border-emerald-800",
  ...
  const desertColor =
    desertRandom > 0.5
      ? "text-yellow-200 bg-yellow-900 border-yellow-800"
      : desertRandom < -0.5
        ? "text-amber-200 bg-amber-800 border-amber-700"
        : "text-orange-300 bg-orange-950 border-orange-900";
...
export const OverlandMap: React.FC<OverlandMapProps> = ({
  ...
  const [autoTravelTargetId, setAutoTravelTargetId] = useState<string | null>(
    null,
  );
  ...
  const handleRefillCanteen = () => {
    onUpdatePlayer((p) => {
      ...
      if (canteenIdx !== -1) {
        updatedInv[canteenIdx].count = Math.max(
          updatedInv[canteenIdx].count,
          10,
        );
      } else {
        updatedInv.push({
          ...
          value: 15,
          count: 10,
          details: "Restores trail hydration automatically.",
        });
      }
      setTimeout(() => {
        addLogMessage(
          "💦 FILLED CANTEEN: Gathered fresh river water (10 swigs stored). Hand-fed horse.",
          "loot",
        );
      }, 50);
      return {
  ...
  const checkChunkExpansion = (px: number, py: number) => {
    ...
    if (newLocs.length > 0) {
      ...
      addLogMessage(
        `🗺️ BADLANDS EXPANSION: ${player.name} crested the far horizon of Sector ${cx},${cy}! Procedurally chartered ${newLocs.length} new landmarks.`,
        "system",
      );
    }
  };
  ...
  // Helper consumption
  const applyStepConsumption = (biome: any, stepHours: number) => {
    onUpdatePlayer((prev) => {
      ...
      if (desertedCount > 0) {
        setTimeout(() => {
          addLogMessage(
            `💸 DESERTION: ${desertedCount} recruited gunfighters abandoned ${prev.name}'s posse because he lacked the gold to pay their turn wages!`,
            "danger",
          );
        }, 50);
      }
      ...
      while (nextHydration < 50) {
        const canteenIdx = inventory.findIndex(
          (item) => item.id === "canteen" && item.count > 0,
        );
        ...
        setTimeout(() => {
          addLogMessage(
            `💦 AUTO-DRINK: You took a swig from your Canteen.`,
            "system",
          );
        }, 50);
      ...
      if (nextHydration <= 0) {
        nextHydration = 0;
        nextHoursDehydrated += stepHours;

        if (nextHasHorse && nextHoursDehydrated >= 72) {
          setTimeout(() => {
            addLogMessage(
              `🐴 TRAGEDY: ${prev.horseName || "Your mount"} collapsed and died from severe dehydration after 3 days in the baking sun!`,
              "danger",
            );
          }, 50);
          nextMount = undefined;
        ...
        } else if (nextHasHorse) {
          const prevHourFloor = Math.floor(prev.hoursDehydrated || 0);
          const nextHourFloor = Math.floor(nextHoursDehydrated);
          if (nextHourFloor > prevHourFloor) {
            if (nextHourFloor === 24) {
              setTimeout(() => {
                addLogMessage(
                  `🐴 THIRSTING HORSE: ${prev.horseName || "Your mount"} is suffering severely from dehydration! (24 hours dehydrated)`,
                  "danger",
                );
              }, 50);
            } else if (nextHourFloor === 48) {
              setTimeout(() => {
                addLogMessage(
                  `🐴 WEAKENING STEED: ${prev.horseName || "Your mount"} is extremely weak and struggling to match your pace! (48 hours dehydrated)`,
                  "danger",
                );
              }, 50);
            }
        ...
        if (nextHoursDehydrated >= 72) {
          ...
          const prevHourFloor = Math.floor(prev.hoursDehydrated || 0);
          const nextHourFloor = Math.floor(nextHoursDehydrated);
          if (prevHourFloor === 0 || (nextHourFloor > prevHourFloor && nextHourFloor === 1)) {
            setTimeout(() => {
              addLogMessage(
                `🥵 DEHYDRATION: You are out of water! Max AP reduced, HP regen halted. Survival clock ticking.`,
                "danger",
              );
            }, 50);
          }
    ...
    setWorldLocations((prev) => {
      ...
      if (newlyRevealedNames.length > 0) {
        newlyRevealedNames.forEach((name) => {
          addLogMessage(
            `🗺️ LANDMARK DISCOVERED: Scouted ${name} through New Mexico mapping!`,
            "loot",
          );
        });
      }
  ...
  const getPlayerSpeed = (player: Player) => {
    ...
    if (player.mount) {
      ...
      baseSpeed = player.mount.baseSpeedMultiplier;
    } else {
    ...
    const hoursDehydrated = player.hoursDehydrated || 0;
    if (hoursDehydrated > 0) {
      const multiplier = Math.max(0, 1 - hoursDehydrated / 72);
  ...
  const handleRescueByStranger = () => {
    onUpdatePlayer((prev) => {
      ...
      const randomSwigs = Math.random() < 0.5 ? 2 : 3;
      const canteenIdx = updatedInv.findIndex((i) => i.id === "canteen");
      if (canteenIdx !== -1) {
        updatedInv[canteenIdx] = {
          ...updatedInv[canteenIdx],
          count: randomSwigs,
        };
      } else {
      ...
      return {
        ...prev,
        hoursDehydrated: 0,
        hydration: 25, // Fill hydration back to 25% to allow immediate normal movement
        inventory: updatedInv,
      };
    });
    ...
    addLogMessage(
      `🤠 MYSTERIOUS STRANGER: Dragged your body into the shade, filled your canteen, and left you with a message of hope.`,
      "loot",
    );
  };
  ...
  const processOverlandStep = (
    ...
    const shouldRestDayTravel =
      !travelByNight && (gameTimeHour >= 21 || gameTimeHour < 6.95);
    const shouldRestNightTravel =
      travelByNight && gameTimeHour >= 7.05 && gameTimeHour < 16.95;
    ...
    if (prevZone !== nextZone) {
      if (nextZone === "bandit") {
        const feelings = [
          ...
          "Your horse whinnies nervously.",
          player.posse.length > 0
            ? `${player.posse[0].name} mutters: "I've heard bad things about this area."`
            : "You quickly check your revolver's cylinder.",
        ];
        addLogMessage(
          `⚠️ ${feelings[Math.floor(Math.random() * feelings.length)]} (Entered Bandit Territory)`,
          "danger",
        );
      } else if (nextZone === "tribe") {
        addLogMessage(
          `⛺ Entered Native Tribal Lands. Keep your hands where they can see them.`,
          "system",
        );
      } else {
    ...
    const stepHours = player.hasHorse ? 0.3 : 0.6;
    ...
    const retaliationBonusChance =
      outlawRep < -10 ? (Math.abs(outlawRep) / 100) * 0.1 : 0;
    ...
    if (activeNemeses.length > 0 && Math.random() < 0.03) {
      ...
      addLogMessage(
        `💀 NEMESIS AMBUSH: ${nemesis.name} has tracked you down to exact their revenge!`,
        "danger",
      );
    ...
    const activeInvestigationsFinal = player.inventory
      .map((invItem) => {
    ...
    for (const mission of activeInvestigationsFinal) {
      if (
        Math.hypot(
          nextX - mission.hiddenTargetHex[0],
          nextY - mission.hiddenTargetHex[1],
        ) < 4.0
      ) {
        if (isAutoTraveling) {
          setIsAutoTraveling(false);
        ...
        if (
          mission.twistType !== "STANDARD" &&
          mission.questState === "TWIST_REVEALED"
        ) {
          addLogMessage(
            `⚠️ CONFRONTATION: You found ${mission.targetName}'s hideout. Time to make a choice.`,
            "danger",
          );
          onConfrontBoss(mission);
        } else {
          addLogMessage(
            `⚠️ BOUNTY LOCATED: You reached ${mission.targetName}'s precise hideout. Draw weapons!`,
            "danger",
          );
    ...
    const activeInvestigations = player.inventory
      .map((invItem) => {
        let mission: any = null;
        for (const l of locations) {
          const q = l.quests?.find((quest) => quest.id === invItem.id);
    ...
    if (activeInvestigations.length > 0 && Math.random() < 0.08) {
      const mission =
        activeInvestigations[
          Math.floor(Math.random() * activeInvestigations.length)
        ];
      const distToHidden = Math.hypot(
        nextX - mission.hiddenTargetHex[0],
        nextY - mission.hiddenTargetHex[1],
      );
      if (distToHidden <= 25) {
        if (!locations.some((l) => l.id === `track_${mission.id}`)) {
          ...
          addLogMessage(
            `👣 INVESTIGATION: You noticed signs of an outlaw passing through nearby. A 'Disturbed Campfire' appeared on the map! Walk to it to gather a clue.`,
            "system",
          );
        }
      }
    ...
    if (pickUpTrackId) {
      ...
      addLogMessage(
        `🔍 CLUE FOUND: Examined the campfire. Learned more about the location of ${targetName !== "" ? targetName : "the target"}. The marking glow tightened on your map!`,
        "loot",
      );

      if (twistRevealed) {
        addLogMessage(
          `⚠️ INVESTIGATION TWIST: Some items discarded at the camp point to an alarming motive... Check the map thoroughly!`,
          "danger",
        );
      }
    ...
    if (Math.random() < eventChance) {
      ...
      if (isNightAttackOnNPC) {
        addLogMessage(
          `🌙 NIGHT SURPRISE: You got the drop on an outlaw camp in the dark! Confiscated $15 gold!`,
          "loot",
        );
        ...
        if (isRetaliation) {
          addLogMessage(
            `💀 FACTION RETALIATION: An outlaw gang hit squad ambushed you on the open trail!`,
            "danger",
          );
          ...
          addLogMessage(
            `⚔️ BANDIT AMBUSH: Outlaws spotted your trail and fired a warning shot! You lose 5 HP!`,
            "danger",
          );
        ...
        addLogMessage(
          `⚠️ TREACHEROUS TERRAIN: You slipped and suffered an injury in the harsh ${biome.name}! (-5 HP)`,
          "danger",
        );
        onUpdatePlayer((p) => ({ ...p, hp: Math.max(1, p.hp - 5) }));
      } else if (roll < 0.1) {
        addLogMessage(
          `🚂 You pass a chain gang of prisoners building a railroad out in the dust. The guards nod at you.`,
          "system",
        );
      } else if (roll < 0.2) {
        addLogMessage(
          `🤠 A traveling trader crosses your path. He tosses you an old canteen!`,
          "loot",
        );
      } else if (roll < 0.3) {
        const foundGold = 10 + Math.floor(Math.random() * 20);
        addLogMessage(
          `🏚️ You found a lost, abandoned hacienda! You search the old pots and find $${foundGold}.`,
          "loot",
        );
        onUpdatePlayer((p) => {
        ...
        onUpdatePlayer((p) => {
          ...
          if (badgeIndex !== -1) updatedInv[badgeIndex].count += 1;
            ...
            updatedInv.push({
              ...
              count: 1,
              details:
                "Restores 35 HP on use. Crucial for stemming arterial bleeding. (Weight: 0.5 lbs)",
            });
          return { ...p, inventory: updatedInv };
        });
        addLogMessage(
          `🛠️ FOUND SUPPLIES: You scavenged an old clean bandage from a ruined wagon!`,
          "loot",
        );
      } else if (roll < 0.7) {
        addLogMessage(
          `💦 TRAILSIDE CACHE: Located some clear water under high shade peaks.`,
          "loot",
        );
      } else {
        addLogMessage(
          `☀️ ARIZONA HEATWAVE: Suffocated under extremely hot dry air gusts.`,
          "danger",
        );
      }
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (isAutoTraveling) {
        ...
        addLogMessage(
          "⌨️ MANUAL OVERRIDE: Switched back to manual badlands exploration.",
          "system",
        );
      }
  ...
  const handleAutoTravelTo = (targetLoc: Location) => {
    ...
    if (!visited.includes(targetLoc.id)) {
      addLogMessage(
        `🔒 LOCKED DESTINATION: You haven't explored/visited ${targetLoc.name} yet! Walk there manually using keys first.`,
        "danger",
      );
      return;
    }
    ...
    addLogMessage(
      `🏇 AUTO-TRAVEL ENGAGED: ${player.name} began auto-traveling towards ${targetLoc.name}. You can break or override anytime using W, A, S, D, Q, E, Y, C.`,
      "travel",
    );
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (distanceToTarget < 2.5) {
        ...
        addLogMessage(
          `🏇 ARRIVED: Reached your known target destination: ${targetLoc.name}.`,
          "travel",
        );
        return;
      }
  ...
  const getLocationTypeName = (type: Location["type"]) => {
    switch (type) {
      case "boomtown":
        return "Boomtown (Trading Settlement)";
      case "railway_hub":
        return "Railway Depot (Transport Hub)";
      case "outlaw_haven":
        return "Outlaw Refuge (Black Market)";
      case "ghost_town":
        return "Decaying Ghost Town";
      case "mine":
        return "Abandoned Gold Mine Dungeon";
      case "desert_oasis":
        return "Arid Desert Oasis (Free Springs)";
      case "hostile_camp":
  ...
  const getViewportCoords = (gx: number, gy: number) => {
    ...
    return {
      x: rx,
      y: ry,
      visible: rx >= -20 && rx <= 120 && ry >= -20 && ry <= 120,
    };
  };
    ...
    >
      ...
      <div className="lg:col-span-8 flex flex-col bg-[#f4ead5] border border-[#bfae96] rounded-sm p-4 h-full min-h-[480px]">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                gameTimeHour >= 6 && gameTimeHour < 19
          ...
          <svg
            className="w-full h-full absolute inset-0 pointer-events-none mix-blend-overlay opacity-20"
            viewBox="0 0 100 100"
          ...
          >
            <defs>
              ...
              >
                <path
                  d={`M ${(4 / CAMERA_VIEW_SIZE) * 100} 0 L 0 0 0 ${(4 / CAMERA_VIEW_SIZE) * 100}`}
                  fill="none"
                  stroke="#8c6b0c"
                  strokeWidth="0.5"
                />
          ...
          {passedSectors.map((key) => {
            ...
            if (biome.name === "Savannah Plains") {
              ...
              if (randFrac > 0.5)
                iconToRender = (
                  <span className="font-bold text-amber-900/40">◠</span>
                );
          ...
          {(player.hydration ?? 100) <= 0 && (
            <div className="absolute top-12 left-4 z-40 bg-[#251010]/95 border-2 border-red-650 p-2.5 rounded-sm shadow-xl max-w-[240px] flex flex-col gap-1.5 font-serif text-xs text-red-200 border-red-800">
              <div className="flex items-center gap-1.5 font-bold uppercase tracking-wider text-red-400 justify-between">
                ...
                <span className="text-[8px] font-mono bg-red-950 px-1.5 py-0.5 rounded text-red-300 font-bold">
                  {(player.hoursDehydrated || 0).toFixed(1)}h elapsed
                </span>
              </div>
              <p className="font-sans text-[9px] text-red-300 leading-normal">
                Your canteens are dry. You suffer from sun stroke. Speed decreases hourly, reaching 0 on day 3 (72h).
              </p>
              <div className="space-y-1 font-mono text-[8.5px] text-[#cca0a0] border-t border-red-950/50 pt-1">
                <div className="flex justify-between border-b border-red-950/30 pb-0.5">
                  <span>🏇 Movement Speed:</span>
                  <span className="text-red-400 font-bold">
                    {Math.round(Math.max(0, 1 - (player.hoursDehydrated || 0) / 72) * 100)}%
                  </span>
                </div>
                <div className="flex justify-between border-b border-red-950/30 pb-0.5">
                  <span>⚡ Action (AP):</span>
                  <span className="text-red-400 font-bold">-50% Max AP</span>
                </div>
                <div className="flex justify-between border-b border-red-950/30 pb-0.5">
                  <span>⛺ Camping:</span>
                  <span className="text-red-400 font-bold">Halted HP Recovery</span>
                </div>
                <div className="flex justify-between">
                  <span>🤠 Ambush Risk:</span>
                  <span className="text-red-400 font-bold">Elevated Risk (+20%)</span>
                </div>
                {player.hasHorse && (
                  <div className="border-t border-red-950/30 pt-1 mt-1 text-[8px] text-red-300">
                    🐴 Mount survival:{" "}
                    <span className="font-bold text-red-400">
                      {Math.max(0, 72 - Math.floor(player.hoursDehydrated || 0))}h remaining
                    </span>
                  </div>
                )}
              </div>
          ...
          {(() => {
            ...
            for (
              let x = startGX;
              x < cameraX + CAMERA_VIEW_SIZE + snap;
              x += snap
            ) {
              for (
                let y = startGY;
                y < cameraY + CAMERA_VIEW_SIZE + snap;
                y += snap
              ) {
                const mountainVal = Math.sin(x / 30) * Math.sin(y / 30);
                if (mountainVal > 0.25) {
          ...
          >
            ...
            {(() => {
              ...
              for (
                let vx = startGX;
                vx <= cameraX + CAMERA_VIEW_SIZE + snap;
                vx += snap
              ) {
                const riverVal =
                  Math.sin(vx / 25) * 40 + Math.cos(vx / 40) * 15;
            ...
            {railwayHubs.map((hubA, i) =>
              railwayHubs.slice(i + 1).map((hubB) => {
                const isAExplored =
                  hubA.isExplored || hubA.id === currentLocationId;
                const isBExplored =
                  hubB.isExplored || hubB.id === currentLocationId;
            ...
            {selectedLocation &&
              ...
              (() => {
                ...
                const coordsTarget = getViewportCoords(
                  selectedLocation.x,
                  selectedLocation.y,
                );
                return (
          ...
          {player.inventory.map((invItem) => {
            ...
            if (!mission || mission.questState === "COMPLETED_BOUNTY")
              return null;
            ...
            if (clues === 1) {
              return (
                <div
                  ...
                  className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-red-500/50 bg-red-500/20 pointer-events-none z-10 animate-pulse"
                  style={{
                    left: `${bossCoords.x}%`,
                    top: `${bossCoords.y}%`,
                    width: "25%",
            ...
            if (clues === 2) {
              return (
                <div
                  key={`investigate-${mission.id}`}
                  className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-red-600/70 bg-red-500/30 pointer-events-none z-10 animate-pulse"
                  style={{
                    left: `${bossCoords.x}%`,
                    top: `${bossCoords.y}%`,
                    width: "7.5%",
            ...
            return (
              ...
              >
                ...
                <div className="mt-1 flex flex-col items-center pointer-events-none z-10">
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded-sm select-none tracking-wide font-sans ${
                      isCurrent
            ...
            <div className="relative flex justify-center items-center">
              ...
              {player.hydration <= 0 && !showCampFire && (
                <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 flex flex-col items-center z-50">
                  <span className="text-2xl drop-shadow-[0_0_15px_rgba(220,38,38,1)] animate-bounce text-red-500">
                    🥵
                  </span>
                  <span className="text-[8px] font-bold uppercase text-red-100 bg-[#3a0505] border border-red-500/50 px-1.5 py-0.5 rounded-sm mt-0.5 whitespace-nowrap shadow-lg leading-none">
                    Thirsty
                  </span>
                </div>
              )}
              {showCampFire && (
                <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 flex flex-col items-center z-50 transition-opacity">
                  <span className="text-2xl drop-shadow-[0_0_15px_rgba(249,115,22,1)] animate-bounce">
                    ⛺
                  </span>
                  <span className="text-[9px] font-bold uppercase text-orange-400 bg-black/80 px-2 py-0.5 rounded-sm mt-1 whitespace-nowrap shadow-md">
                    Resting
                  </span>
                </div>
              )}
          ...
          {/* Mysterious Stranger Rescue Modal */}
          {showStrangerModal && (
            <div className="absolute inset-0 z-[100] bg-black/85 backdrop-blur-xs flex items-center justify-center p-6 animate-fade-in pointer-events-auto">
              <div className="bg-[#2d1b11] border-4 border-[#c59b27] rounded shadow-[0_0_25px_rgba(197,155,39,0.4)] max-w-sm w-full p-5 space-y-4 text-center font-serif relative">
                <div className="space-y-1.5">
                  <div className="text-xl text-[#e8c33a] font-extrabold uppercase tracking-widest flex justify-center items-center gap-2">
                    🏜️ MYSTERIOUS STRANGER
                  </div>
                  <div className="h-0.5 w-1/2 bg-gradient-to-r from-transparent via-[#c59b27] to-transparent mx-auto"></div>
                </div>

                <div className="space-y-3">
                  <p className="text-[11px] text-[#eedec4] leading-relaxed font-sans italic">
                    You collapsed completely in the scorching badlands sand under the noon heat, your vision fading to black...
                  </p>
                  <p className="text-[11px] text-[#f4ead5] leading-relaxed font-sans mt-2">
                    A lone silhouette approached you. He knelt beside your bone-dry lips, poured refreshing glacier water from a silver flask down your parched throat, and wiped the desert grit from your face.
                  </p>
                  <p className="text-[10px] text-amber-300 leading-normal font-sans mt-2">
                    The stranger filled your trail canteen up with fresh well water, gave you a polite tip of his Stetson hat, and dissolved into the shimmering heat haze.
                  </p>
                </div>
                ...
                <div className="bg-[#1e110a] border border-[#c59b27]/30 p-2.5 rounded-sm space-y-1.5 text-left font-mono text-[9px] text-amber-200">
                  ...
                  <div className="flex justify-between">
                    <span>💧 Hydration:</span>
                    <span className="text-sky-300 font-bold">+25% Restored</span>
                  </div>
                  <div className="flex justify-between">
                    <span>🎒 Trail Canteen:</span>
                    <span className="text-sky-300 font-bold">
                      +{player.inventory.find(i => i.id === "canteen")?.count || 2} Swigs Filled (Random)
                    </span>
                  </div>
                </div>

                <button
                  onClick={handleRescueByStranger}
                  className="w-full py-2.5 bg-[#8c6b0c] hover:bg-[#cbae82] border-t-2 border-b-2 border-amber-400 text-white hover:text-black font-extrabold uppercase tracking-wide text-[10px] transition-colors cursor-pointer rounded-sm"
                >
      ...
      <div className="lg:col-span-4 flex flex-col gap-4 bg-[#f4ead5] border border-[#bfae96] p-5 rounded-sm h-full justify-between shadow-xl">
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-[#dcd1b9] border border-[#bfae96]/60 rounded-sm p-3.5 mb-4 space-y-2">
            <div className="flex justify-between items-center">
              ...
              <div
                className={`px-2 py-0.5 rounded text-[9px] uppercase font-mono border ${getBiomeAt(playerX, playerY).color}`}
              >
                {getBiomeAt(playerX, playerY).icon}{" "}
                {getBiomeAt(playerX, playerY).name}
              </div>
            </div>
            ...
            <div className="flex justify-between text-[10px] font-mono text-[#4a3928]">
              ...
              <span className="text-[#2d2119] capitalize">
            ...
            <div className="border-t border-[#bfae96]/30 pt-2 mt-2 text-[9px] text-[#664d36] uppercase leading-relaxed font-sans">
              🚀 <b>Manual Steering:</b> Use <b>WASD</b> for cardinal movement,{" "}
              <b>Q, E, Y, C</b> for diagonal tracking to navigate manually.
            </div>
          </div>

          {/* TRAIL HYDRATION & VITALS SYSTEM */}
          <div className="bg-[#dfd4bd] border-2 border-[#bfae96] rounded-sm p-4 mb-4 space-y-3.5 shadow-sm">
            <div className="flex justify-between items-center border-b border-[#bfae96]/60 pb-2">
              <span className="text-[10px] text-[#8c6b0c] uppercase font-serif tracking-wider font-extrabold flex items-center gap-1.5">
                💧 Trail Hydration & Vitals
              </span>
              <span
                className={`px-1.5 py-0.5 rounded text-[8px] font-mono border font-bold uppercase transition-all ${
                  player.hydration <= 0
                    ? "bg-red-950/25 border-red-600/60 text-red-700 animate-pulse"
                    : player.hydration < 30
                      ? "bg-amber-950/25 border-amber-500 text-amber-700 font-bold animate-pulse"
                      : "bg-sky-950/25 border-sky-500 text-sky-700"
                }`}
              >
                {player.hydration <= 0
                  ? "🥵 DEHYDRATED"
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[10px] font-mono text-[#5a4838]">
              ...
              <div className="w-full bg-[#c4b59b] rounded-full h-2.5 overflow-hidden border border-[#bfae96]/60 shadow-[inset_0_1px_3px_rgba(0,0,0,0.1)]">
                <div
                  className={`h-full transition-all duration-300 ${
                    player.hydration <= 0
                      ? "bg-red-600 shadow-[0_0_8px_rgba(220,10,10,0.5)]"
                      : player.hydration < 30
                        ? "bg-gradient-to-r from-amber-600 to-amber-500 animate-pulse"
                        : "bg-gradient-to-r from-sky-600 to-sky-450"
                  }`}
                  style={{
                    width: `${Math.max(0, Math.min(100, (player.hydration / (player.maxHydration ?? 100)) * 100))}%`,
                  }}
                />
              </div>
            </div>

            {/* Trail Canteen display inside the widget */}
            {(() => {
              const canteenItem = player.inventory.find(
                (item) => item.id === "canteen",
              );
              const swigs = canteenItem ? canteenItem.count : 0;
              return (
                <div className="bg-[#bfae96]/20 border border-[#bfae96]/50 rounded-sm p-2 flex justify-between items-center text-[10px] font-mono text-[#4a3928]">
                  <span className="flex items-center gap-1">
                    🎒 Trail Canteen:
                  </span>
                  <span
                    className={`font-bold ${swigs === 0 ? "text-red-700" : "text-sky-850"}`}
                  >
                    {swigs > 0
                      ? `💦 ${swigs} swigs remaining`
                      : "❌ Completely Empty"}
                  </span>
                </div>
              );
            ...
            <div className="space-y-1.5 border-t border-[#bfae96]/40 pt-2 mt-1">
              ...
              {player.hydration <= 0 ? (
                <div className="space-y-2 bg-red-950/10 border border-red-920/40 p-2.5 rounded-sm">
                  ...
                  <ul className="text-[9px] font-mono text-[#5f0303] leading-relaxed list-disc list-inside space-y-1 pl-0.5">
                    <li>
                      ...
                      Reduced to <b>{Math.round(Math.max(0, 1 - (player.hoursDehydrated || 0) / 72) * 100)}%</b> of normal (reaches 0 on day 3).
                    </li>
                    <li>
                      <span className="text-red-700 font-bold">
                        Combat Reflexes:
                      </span>{" "}
                      Starts & retrieves <b>-50% Action Points (AP)</b> in gun
                      duels and tactical setups.
                    </li>
                    <li>
                      <span className="text-red-700 font-bold">
                        Resting Recovery:
                      </span>{" "}
                      Halted. Health <b>regen from camping</b> is completely
                      blocked.
                    </li>
                    <li>
                      <span className="text-red-700 font-bold">
                      ...
                      Increased risk. <b>+20% Ambush Chance</b> while resting in
                      the badlands.
                    </li>
                    {player.hasHorse && (
                      <li className="bg-red-50 text-red-900 px-1 py-1 rounded border border-red-200 mt-1.55">
                        <span className="text-red-700 font-bold">
                          🌵 Mount Threat:
                        </span>{" "}
                        Horse death in{" "}
                        <b>
                          {Math.max(0, 72 - Math.floor(player.hoursDehydrated || 0))}{" "}
                          hours
                        </b>{" "}
                        of continuous dehydration!
                        {player.hoursDehydrated &&
                        player.hoursDehydrated >= 24 ? (
                          <span className="block text-[8.5px] text-red-600 font-semibold mt-1">
                            ⚠️ {player.horseName || "Your horse"} is suffering
                            severely! ({player.hoursDehydrated}h elapsed)
                          </span>
                        ) : null}
                      </li>
                    )}
                ...
                <div className="bg-[#e8dec7]/60 border border-[#bfae96]/30 p-2 rounded-sm space-y-1 text-[9px] font-mono text-[#5a4838]">
                  <div className="text-emerald-800 font-bold uppercase flex items-center gap-1">
                    🟢 TRAIL FIT & COMFORTABLE:
                  </div>
                  <p className="leading-normal">
                    You have ample hydration. Normal movement speed is active,
                    stamina functions at 100%, and resting heals you fully.
                  </p>
                  {player.hydration < 30 && (
                    <p className="text-amber-700 font-bold leading-normal border-t border-[#bfae96]/20 pt-1 mt-1 font-sans">
                      ⚠️ Warning: Hydration is dipping dangerously. Canteen will
                      drink automatically if low, but secure water soon!
                    </p>
                  )}
                </div>
              )}
          ...
          {(() => {
            const nearLocation = locations.find((loc) => {
              return (
                loc.isExplored &&
                Math.hypot(loc.x - playerX, loc.y - playerY) < 3.5
              );
            });
            ...
            return (
              <div className="bg-[#1e1511] border-2 border-[#e8b923] p-3 rounded-sm mb-4 space-y-2 text-center shadow-lg">
                ...
                <p className="text-[9px] text-[#4a3928] leading-normal font-sans">
                  Saddle down and click the button below to dismount and enter
                  buildings, offices and saloons here.
                </p>
                <button
                  ...
                  onClick={() => {
                    onUpdatePlayer((prev) => {
                      ...
                      if (!list.includes(nearLocation.id)) {
                        return {
                          ...prev,
                          visitedLocationIds: [...list, nearLocation.id],
                        };
                      }
                      return prev;
                    });
          ...
          {!selectedLocation ? (
            <div className="flex-1 flex flex-col justify-center items-center py-10 text-center px-4 space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-[#8c6b0c] font-serif text-xs font-bold uppercase tracking-wider">
                  Map Navigation Ready
                </h3>
                <p className="text-[#4a3928] text-[11px] leading-relaxed max-w-[240px] font-sans">
                  Scout individual desert districts on the chart map to track
                  risks, secure spring coordinates, trade, and mount up.
                </p>
              </div>

              {/* Current Station status box */}
              <div className="w-full bg-[#dfd4bd] border border-[#bfae96] rounded-sm p-3 text-left space-y-1.5">
                <div className="text-[9px] text-[#664d36] uppercase tracking-wider font-serif">
                  Current Outpost
                </div>
                <div className="text-[#8c6b0c] font-serif font-bold text-xs">
                  {currentLocation.name}
                </div>
                <div className="text-[#4a3928] text-[10px] leading-relaxed italic font-sans">
            ...
            (() => {
              const isSelectedRevealed =
                selectedLocation.isExplored ||
                selectedLocation.id === currentLocationId;
              const isSelectedQuestTarget = selectedLocation.quests.some(
                (quest) =>
                ...
                <div className="space-y-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="bg-[#dfd4bd] p-4 border border-[#bfae96] rounded-sm space-y-3.5">
                    <div className="flex justify-between items-start border-b border-[#bfae96]/60 pb-2">
                      <div>
                        <span
                          className={`text-[9.5px] font-serif uppercase tracking-widest font-bold ${getLocationColorClass(selectedLocation.type)}`}
                        >
                          {isSelectedRevealed
                            ? getLocationTypeName(selectedLocation.type)
                            : "Obscured Sector"}
                        </span>
                        <h3 className="text-[#8c6b0c] font-serif font-bold text-base mt-0.5">
                          {selectedDisplayName}
                        </h3>
                      </div>
                      {selectedLocation.id === currentLocation.id && (
                    ...
                    <p className="text-[#4a3928] text-xs italic leading-relaxed font-sans">
                      {selectedDisplayDesc}
                    </p>
                      ...
                      <div className="flex justify-between text-xs font-sans">
                        ...
                        <span
                          className={`font-mono font-bold ${
                            !isSelectedRevealed
                              ? "text-zinc-500"
                              : selectedLocation.risk < 0.35
                                ? "text-emerald-400"
                                : selectedLocation.risk < 0.7
                                  ? "text-[#8c6b0c]"
                                  : "text-[#c4451a] animate-pulse"
                          }`}
                        >
                          {!isSelectedRevealed
                            ? "Obscured"
                            : selectedLocation.risk >= 0.7
                              ? "CRITICAL LETHAL"
                              : `${Math.round(selectedLocation.risk * 100)}% Danger`}
                        </span>
                      </div>

                      <div className="flex justify-between text-xs font-sans">
                        <span className="text-[#664d36]">
                          Bank Vault Reserve:
                        </span>
                        <span className="text-[#8c6b0c] font-mono font-semibold text-right">
                          {!isSelectedRevealed
                            ? "Unknown"
                            : selectedLocation.bankGold > 0
                              ? `$${selectedLocation.bankGold} Nuggets`
                              : "Exhausted"}
                        </span>
                      </div>
                    </div>
                  ...
                  {selectedLocation.id !== currentLocation.id && (
                    <div className="space-y-3 pt-2">
                      <span className="text-[10px] uppercase font-serif tracking-widest text-[#664d36] block">
                      ...
                      <div className="bg-[#e8dec7] border border-[#bfae96] rounded-sm p-3.5 space-y-2.5">
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-[#3d2d21] font-bold font-serif flex items-center gap-1.5">
                            {player.hasHorse
                              ? "🐴 Mount Kentucky Stallion"
                              : "🚶 Travel on Foot"}
                          </span>
                          <span className="text-[10px] text-[#664d36] font-serif font-mono">
                            {travelDistance} Miles
                          </span>
                        </div>

                        {player.visitedLocationIds?.includes(
                          selectedLocation.id,
                        ) ? (
                          <>
                            <div className="flex flex-col gap-1 text-[11px] font-sans border-t border-[#bfae96]/40 pt-2 pb-1 text-[#4a3928] leading-relaxed">
                              <p>
                                This destination is <b>Visited & Known</b>.
                                Engaging auto-travel will automatically steer $
                                {player.name} one sector step at a time,
                                consuming supplies based on the local biome.
                              </p>
                            </div>
                          ...
                          <div className="border-t border-[#bfae96]/50 pt-2 space-y-2">
                            <div className="bg-amber-950/20 border border-amber-900/60 p-2.5 rounded-sm text-[10px] text-amber-400 font-sans leading-relaxed">
                              🔒 <b>UNCHARTED OUTPOST:</b> You have not visited
                              this location yet. Auto-travel is locked. You must
                              manually scout the badlands terrain using standard
                              key commands <b>[W, S, A, D] or [Q, E, Y, C]</b>{" "}
                              to reach and discover this point of interest
                              first!
                            </div>
                          </div>
                        )}
                      ...
                      <div className="bg-[#e8dec7] border border-[#bfae96] rounded-sm p-3.5 space-y-2.5">
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="text-[9px] bg-[#2a8ec4]/10 text-[#2a8ec4] border border-[#2a8ec4]/30 px-1.5 py-0.2 rounded-sm font-serif uppercase">
                            Instant / 100% Safe
                          </span>
                        </div>
                        ...
                        {worksWithTrain ? (
                          <>
                            <div className="flex justify-between items-center text-[11px] font-sans">
                              <span className="text-[#4a3928]">
                                Railway Passenger Fare:
                              </span>
                              <span
                                className={`${canAffordTrain ? "text-[#8c6b0c]" : "text-[#c4451a] font-bold"} font-mono`}
                              >
                            ...
                            <button
                              ...
                              onClick={() => {
                                addLogMessage(
                                  `🚨 TRAIN HEIST: You drew your iron and stopped the Railway Express! Guards are opening fire!`,
                                  "danger",
                                );
                                onStartCombat(
                                  "train_robbery",
        ...
        <div className="mt-4 pt-3 border-t border-[#bfae96] font-serif text-[10px] text-[#8a7560] leading-relaxed text-center leading-normal">
          ⚡ DESERTS ARE SCORCHING: Outposts in deep desert terrain trigger
          aggressive scorpion raids.
        </div>
      </div>
    </div>
  );
